...
import { Settings2, Save, Moon, Sun, Monitor, Key, Eye, EyeOff, Trash2, CheckCircle2, AlertCircle, ChevronDown, ChevronRight } from "lucide-react";
...
const PROVIDER_META: Record<Provider, { label: string; icon: React.ReactNode; color: string; description: string; placeholder: string; docsUrl: string }> = {
  openai: {
    ...
    description: "Access GPT-4o, o3-mini, o1, and more",
    placeholder: "sk-proj-...",
    docsUrl: "https://platform.openai.com/api-keys",
  },
  anthropic: {
    label: "Anthropic",
    icon: <span className="text-[#c07945] font-bold text-sm">A</span>,
    color: "text-orange-400",
    description: "Access Claude 3.5 Sonnet, Haiku, Opus",
    placeholder: "sk-ant-...",
    docsUrl: "https://console.anthropic.com/settings/keys",
  },
  openrouter: {
    label: "OpenRouter",
    icon: <span className="text-purple-400 font-bold text-sm">OR</span>,
    color: "text-purple-400",
    description: "Access Llama 3.3, DeepSeek R1, Phi-4, Qwen and many more",
    placeholder: "sk-or-...",
...
function ProviderKeyRow({ provider, maskedKey, onSave, onDelete }: {
  ...
  return (
    <div className={cn(
      ...
      hasKey ? "border-primary/30 bg-primary/5" : "border-border/50 bg-card/30"
    )}>
      <div
      ...
      >
        ...
        {hasKey && (
          <span className="text-xs font-mono text-muted-foreground hidden sm:block">{maskedKey}</span>
        )}
      ...
      {expanded && (
        <div className="px-4 pb-4 border-t border-border/30 pt-4 space-y-3">
          <p className="text-xs text-muted-foreground">
            ...
            <a href={meta.docsUrl} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">
              {meta.docsUrl.split("/")[2]}
            </a>.
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              data-testid={`button-save-key-${provider}`}
            ...
            {hasKey && (
              <button
                ...
                disabled={deleting}
                data-testid={`button-delete-key-${provider}`}
...
export default function SettingsPage() {
  ...
  const upsertKey = useUpsertUserApiKey();
  ...
  const handleSaveKey = async (provider: Provider, key: string) => {
    await upsertKey.mutateAsync({ provider, data: { key } });
      ...
      <div className="max-w-2xl mx-auto space-y-8">
        ...
        <div className="bg-glass rounded-2xl p-6 space-y-5">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-2">
              Gemini models are built-in (no key needed). Add your own API keys to unlock OpenAI, Anthropic, and OpenRouter models.
            </p>
          </div>

          {/* Gemini built-in indicator */}
        ...
        <div className="bg-glass rounded-2xl p-6 space-y-5">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1.5">Add provider keys above to unlock more models in this selector.</p>
          </div>
          <div>
            ...
            />
            <p className="text-xs text-muted-foreground mt-1.5">Provided to the AI before every conversation.</p>
          </div>
        </div>
